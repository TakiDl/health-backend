...
//             {
//                 $or: [
//                     { supervised_patients: patientId },
//                     { pending_requests: patientId }
//                 ]
//             },
//             {
//                 $pull: {
...
// // ==========================================
// // --- 🌟 UPGRADED AUTHENTICATION (SIGNUP & LOGIN) 🌟 ---
// // ==========================================
// app.post('/signup', upload, async (req, res) => {
//     try {
//         const certFile = req.files && req.files['certificate'] ? req.files['certificate'][0] : null;
...
//         // 🌟 Added 'email' to extracted body
//         const { name, email, username, password, role, age, weight, height, plan } = req.body;

//         // 🌟 Smart duplicate check: Search by username OR email
//         const existingPatient = await Patient.findOne({ $or: [{ username }, { email }] });
//         const existingExpert = await Expert.findOne({ $or: [{ username }, { email }] });

//         if (existingPatient || existingExpert) {
//             return res.status(400).json({ message: "Username or Email already exists. Please try another." });
//         }
...
//             const newPatient = new Patient({
//                 name, email, username, password: hashedPassword, age, weight, height, plan: 'free'
//             });
//             await newPatient.save();
...
// app.post('/login', async (req, res) => {
//     try {
//         // 🌟 Uses a single 'identifier' field (can be Email OR Username)
...
//         // 🌟 Search Admin
//         user = await Admin.findOne({ $or: [{ email: identifier }, { username: identifier }] });
//         if (user) role = 'admin';

//         // 🌟 Search Expert
//         if (!user) {
//             user = await Expert.findOne({ $or: [{ email: identifier }, { username: identifier }] });
//             if (user) {
//                 role = 'expert';
//                 if (user.isVerified === false) {
//                     return res.status(403).json({ message: "Account pending approval. Please wait for Admin verification." });
...
//         // 🌟 Search Patient
//         if (!user) {
//             user = await Patient.findOne({ $or: [{ email: identifier }, { username: identifier }] });
//             if (user) {
//                 role = 'client';
//                 plan = user.plan;
//             }
...
//         const isPasswordValid = await bcrypt.compare(password, user.password);
//         if (!isPasswordValid) return res.status(401).json({ message: "Invalid email/username or password" });
...
//             patientId,
//             updateData,
//             { new: true, strict: false } // 🌟 Ensured strict: false here too for complete safety
//         ).select('-password');
...
//         // 🌟 FIXED: Added strict: false to bypass schema limitations safely!
//         const updatedExpert = await Expert.findByIdAndUpdate(req.params.id, updateData, { new: true, strict: false }).select('-password');
//         res.json({ message: "Expert profile updated successfully!", expert: updatedExpert });
//     } catch (err) {
//         res.status(500).json({ error: err.message });
//     }
...
app.post('/login', async (req, res) => {
    try {
        ...
        if (!user) {
            user = await Expert.findOne({ email: cleanEmail });
            if (user) {
                role = 'expert';
                if (user.isVerified === false) {
                    return res.status(403).json({ message: "Account pending approval. Please wait for Admin verification." });
        ...
        if (!user) {
            user = await Patient.findOne({ email: cleanEmail });
            if (user) {
                role = 'client';
                plan = user.plan;
            }
...
app.get('/payment-requests', async (req, res) => {
    try {
        const requests = await PaymentRequest.find({ status: 'pending' }).populate('userId', 'name username plan email');
        res.json(requests);
    } catch (error) {
        res.status(500).json({ message: "Error fetching requests" });
    }
...
app.get('/experts', async (req, res) => {
    try {
        const experts = await Expert.find().populate('supervised_patients', 'name username email').select('-password');
        res.json(experts);
    } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.put('/patient/:id/profile', async (req, res) => {
    try {
        ...
        if (activity) patient.activity = activity;
        if (daily_budget !== undefined) patient.daily_budget = Number(daily_budget);
        if (email !== undefined) patient.email = email.trim().toLowerCase();
        if (phone !== undefined) patient.phone = phone;
        if (goal_intention !== undefined) patient.goal_intention = goal_intention;
...
app.get('/patient/dashboard/:id', async (req, res) => {
    try {
        ...
        const dashboardData = {
            name: patient.name,
            username: patient.username,
            email: patient.email, // 🌟 Ensure email is returned to the dashboard
            age: patient.age,
            height: patient.height,
